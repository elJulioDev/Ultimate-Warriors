import time
from config import Config
from utils.constants import nivel_carga # Asegúrate de tener esta utilidad

class TransformationManager:
    def __init__(self, input_manager):
        self.input_manager = input_manager
        self.last_transform_attempt = 0
        self.last_s_press = 0.0

    def transform_logic(self, bot, enemy):
        """Decide y ejecuta transformaciones"""
        if bot.acciones.get("golpe") or bot.acciones.get("patada"):
            return

        ahora = time.time()
        
        # Validar cooldown dinámico
        cooldown_base = 2.0
        cooldown_extra = (bot.maxima_transformacion - bot.cap_form_actual) * 1.2
        cooldown_actual = min(12.0, cooldown_base + cooldown_extra)
        
        if ahora - self.last_transform_attempt < cooldown_actual:
            return

        if not bot.puede_transformarse or bot.cantidad_transformaciones < 1:
            return
        if bot.cap_form_actual >= bot.maxima_transformacion:
            return

        # Calcular Score de necesidad
        score = self._calculate_score(bot, enemy)
        umbral = 3.5 + (bot.cap_form_actual * 0.5)
        
        if score < umbral:
            return

        # Seleccionar transformación
        tipo_trans = self._select_transformation(bot, score)
        
        if tipo_trans:
            self._execute_transformation(tipo_trans)
            self.last_transform_attempt = ahora

    def _calculate_score(self, bot, enemy):
        score = 0
        if enemy.damaged > bot.damaged: score += 1
        if enemy.defence > bot.defence: score += 1
        if enemy.speed > bot.speed: score += 1
        if bot.hp < 60 and enemy.hp > bot.hp: score += 2
        if enemy.cap_form_actual > bot.cap_form_actual: score += 1.5
        if enemy.forma_cheat and not bot.forma_cheat: score += 2
        if bot.hp < 30: score += 2
        return score

    def _select_transformation(self, bot, score):
        nivel = nivel_carga(bot.carga)
        costos = Config.COSTO_TRANSFORMACION
        max_f = bot.maxima_transformacion
        
        if bot.cantidad_transformaciones <= 2:
            if nivel >= costos["secuencial"]: return "secuencial"
        else:
            if score >= 6 and nivel >= costos["fase6"] and max_f >= 6: return "fase6"
            elif score >= 5 and nivel >= costos["fase5"] and max_f >= 5: return "fase5"
            elif score >= 4 and nivel >= costos["fase4"] and max_f >= 4: return "fase4"
            elif score >= 3 and nivel >= costos["fase3"] and max_f >= 3: return "fase3"
            elif score >= 2 and nivel >= costos["fase2"] and max_f >= 2: return "fase2"
            elif nivel >= costos["secuencial"]: return "secuencial"
        return None

    def _execute_transformation(self, tipo):
        """Ejecuta la macro de teclas"""
        if time.time() - self.last_s_press < Config.S_COOLDOWN:
            return

        # Secuencia: S (Hold) -> Combo -> S (Release)
        self.input_manager.press("cover")
        time.sleep(0.08) # Wait for cover reg

        if tipo == "secuencial":
            self.input_manager.press_and_release("punch")
        elif tipo == "fase2":
            self.input_manager.press("left")
            self.input_manager.press_and_release("punch")
            self.input_manager.release("left")
        # ... (Implementar resto de fases igual que en script original) ...
        elif tipo == "cheat":
             self.input_manager.press("left")
             self.input_manager.press("right")
             self.input_manager.press_and_release("shot")
             self.input_manager.release("left")
             self.input_manager.release("right")
             
        # Pausa para asegurar input
        time.sleep(0.25) 
        self.input_manager.release("cover")
        self.last_s_press = time.time()